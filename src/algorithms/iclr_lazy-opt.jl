...
function iclr_lazy(problem::StandardLinearProgram, exitcriterion::ExitCriterion; γ=1.0, σ=0.0, R=10, blocksize=10)  # TODO: What is γ and σ?
    ...
    # Micro optimization init
    z_sliced = zeros(C_max)
    q_sliced = zeros(C_max)
    q_hat = zeros(C_max)
    ...
    while !exitflag
        ...
        z_sliced = z[C[j]]                                      # O(C)
        q_sliced = q[C[j]]                                      # O(C)
        Adelta_sliced = A[k] .- A[θ[C[j]]]                      # O(C)

        q_hat = q_sliced + Adelta_sliced .* z_sliced ##         # O(C)
        ...
        x_hat = prox(- 1/γ * q_hat, 1/γ * A[k])                 # O(C)
        ...
        Delta_y = γ * m * a * ((x_hat' * sliced_A_T)' - b[blocks[j]])   # O(C + blocksize)


        time_4 = time()  #


        y[blocks[j]] = y[blocks[j]] + Delta_y                       # O(blocksize)
        pre_a = a
        a = sqrt(1 + σ * A[k] / γ)/(R * m)


        A[k+1] = A[k] + a
        Delta_Delta_y = sliced_A_T * Delta_y                        # O(C + blocksize)

        time_5 = time()  #


        # q[C[j]] = q[C[j]] + (A[k+1] .- A[θ[C[j]] .+ 1]) .* (z[C[j]] + c[C[j]]) + (a + m * pre_a) * Delta_Delta_y ##
        q[C[j]] = q_sliced + Adelta_sliced .* z_sliced + (a + m * pre_a) * Delta_Delta_y ##     # O(C)
        x_tilde[C[j]] = x_tilde[C[j]] + (A[k] .- A[θ[C[j]]]) .* x[C[j]]

        time_6 = time()  #


        z[C[j]] = z[C[j]] + Delta_Delta_y                                           # O(C)
        ...
        x[C[j]] =  prox(x0[C[j]] - 1/γ * q[C[j]], 1/γ * A[k])                           # O(C)
        θ[C[j]] .= k                                                                    # O(C)
